                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="menu_group_id">
                                ...
                                @foreach ($menuGroups as $item)
                                    <option value="{{ $item->id }}" {{ old('menu_group_id', $menuItem->menu_group_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('menu_group_id')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="route">
                                ...
                                @foreach ($routeCollection as $item)
                                    @php $uri = $item->uri(); @endphp
                                    <option value="{{ $uri }}" {{ old('route', $menuItem->route) == $uri ? 'selected' : '' }}>
                                        {{ $uri }}</option>
                                @endforeach
                            </select>
                        </div>
